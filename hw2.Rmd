...
# read in data
musiced <- read_delim("dataset_music_lessons_EF.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
```

```{r}
# data cleaning
musiced_clean <- musiced %>% na.omit() %>% select(-dropout) %>% mutate(auditory_attention_diff = auditory_attention_t1 - auditory_attention_t0, agtb_mx_diff = agtb_mx_t1 - agtb_mx_t0, agtb_cb_diff = agtb_cb_t1 - agtb_cb_t0, inhibition_diff = inhibition_t1 - inhibition_t0, design_fluency_diff = design_fluency_t1 - design_fluency_t0, animal_sorting_diff = animal_sorting_t1 - animal_sorting_t0, clocks_diff = clocks_t1 - clocks_t0) %>% 
    select(sex, age, group, part_training, practice_minutes, education_parents, income_parents, leisure_activities, auditory_attention_diff, agtb_mx_diff, agtb_cb_diff, inhibition_diff, design_fluency_diff, animal_sorting_diff, clocks_diff, bfie, bfia, bfic, bfio, bfin)
```
...
musiced_cv5 <- vfold_cv(musiced, v = 5)
```
